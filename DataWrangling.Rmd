...
```{r}
new_all<- new_all %>%
  mutate(RT = as.numeric(response_time - stimulus_onset))
...
check<- new_all %>%
  group_by(congruency) %>%
  summarise(num_trials = count(congruency),
            mean_accuracy = mean(congruency),
            mean_RT = mean(RT))
...
check2<- new_all %>%
  ...
  group_by(as.character(subject), congruency) %>%
  summarise(num_trials = count(RT),
...
RT_analysis <- new_all %>%
  ...
  summarise(sub_mean = mean(RT))

#accuracy not found because it keeps deleting the things i do.. WHY
...
```{r}
#whats the point of doing the rest of the problems if RT doesn't exist
